...
/**
 * 
 * A "custom/modified" version of the looper class from DaisySP. Added features include:
 * bool returns for start and halfway through recorded loop
 * (NOT IMPLEMENTED YET) variable playback speed.
 * 
*/
...
class cLooper
{
  public:
    ...
    */
    ...
    /** return pair of booleans indicating if loop position is at the beginning or halfway point */
    std::pair<bool, bool> gateLoop() {
        bool is_start = false;
        bool is_halfway = false;
    
        if(state_ != State::EMPTY && pos_ == 0){
            is_start = true;
        } else {
            is_start = false;
        }
        if(state_ != State::EMPTY && pos_ == recsize_ / 2){
            is_halfway = true;
        } else {
            is_halfway = false;
        }
        
        return std::make_pair(is_start, is_halfway);
    }

    /** to do: test playback speed adjustment 
     * \@param speed_factor works 0.0 to ???, where 1.0 is normal speed, anything lower is slower, and higher is faster
    */
    float setSpeed(float speed_factor)
    {
        float inc = 1.f;
        if(half_speed_)
            inc *= 0.5f;
